...
static int print_type_supported(ValueType type)
{
    ...
    if (semantic_type_is_dict(type)) {
        return print_type_supported(semantic_dict_key_type(type)) &&
            print_type_supported(semantic_dict_value_type(type));
    }
    if (semantic_type_is_tuple(type)) {
        for (size_t i = 0; i < semantic_tuple_element_count(type); i++) {
            if (!print_type_supported(semantic_tuple_element_type(type, i))) {
                return 0;
            }
        }
        return 1;
    ...
    if (semantic_type_is_class(type)) {
        for (size_t i = 0; i < semantic_class_field_count(type); i++) {
            if (!print_type_supported(semantic_class_field_type(type, i))) {
                return 0;
            }
        }
        return 1;
    ...
    Scope *scope)
{
    ...
    if (is_print_call(call)) {
        ...
        if (!print_type_supported(arg_type)) {
            semantic_error_at_node(arguments->children[0],
                "print cannot print type %s",
                semantic_type_name(arg_type));
        }
    ...
    Scope *scope)
{
    ...
    if (node->kind == NODE_SLICE) {
        ValueType container_type = semantic_infer_primary_type(info, node->children[0], scope);
        int has_start = !semantic_is_epsilon_node(node->children[1]);
        int has_end = !semantic_is_epsilon_node(node->children[2]);
        ValueType start_type = TYPE_INT;
        ValueType end_type = TYPE_INT;
        ...
        if (has_start) {
            start_type = semantic_infer_expression_type(info, node->children[1], scope);
        }
        if (has_end) {
            end_type = semantic_infer_expression_type(info, node->children[2], scope);
        }
        if (has_start && start_type != TYPE_INT) {
            semantic_error_at_node(node->children[1], "str slice start must be int");
        }
        if (has_end && end_type != TYPE_INT) {
    ...
    if (node->kind == NODE_MATCH_EXPRESSION) {
        type = infer_match_expression_type(info, node, scope);
        semantic_record_node_type(info, node, type);
        return type;
    }
...
static ValueType combine_match_result_types(
    ...
    ValueType branch_type)
{
    if (current_type == 0) {
        return branch_type;
    }
    if (semantic_is_assignable(current_type, branch_type) &&
        semantic_is_assignable(branch_type, current_type)) {
        return current_type;
    }
    if (semantic_is_assignable(branch_type, current_type)) {
        return branch_type;
    }
    if (semantic_is_assignable(current_type, branch_type)) {
        return current_type;
    ...
    Scope *scope)
{
    ...
    for (size_t i = 2; i < node->child_count; i++) {
        const ParseNode *case_node = semantic_expect_child(node, i, NODE_MATCH_CASE);
        const ParseNode *pattern_expr = semantic_expect_child(case_node, 0, NODE_EXPRESSION);
        const ParseNode *value_expr = semantic_expect_child(case_node, 2, NODE_EXPRESSION);
        ValueType branch_type;
        ...
        branch_type = result_type == 0
            ? semantic_infer_expression_type(info, value_expr, scope)
            : semantic_infer_expression_type_with_hint(info, value_expr, scope, result_type);
