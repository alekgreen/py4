...
{   
    ...
    if (look.type == TOKEN_IDENTIFIER) {
        ParseNode *n = create_node(NODE_S, NULL);
        add_child(n, parse_STATEMENT(ts));
        add_child(n, parse_T(ts));
        return n;
    } else if (look.type == TOKEN_EOF) {
        return create_node(NODE_EPSILON, "ε");
    } else {
        return create_node(NODE_EPSILON, "ε");
    }
}
...
{
    Token look = peek_ts(ts);
    if (look.type == TOKEN_NEWLINE) {
        ParseNode *n = create_node(NODE_T, NULL);
        expect(ts, TOKEN_NEWLINE);
        add_child(n, create_node(NODE_TERMINAL, "\\n"));
        add_child(n, parse_S(ts));
        return n;
    }
    return create_node(NODE_EPSILON, "ε");
}
...
{
    ParseNode *n = create_node(NODE_STATEMENT, NULL);
    Token id = expect(ts, TOKEN_IDENTIFIER);
    add_child(n, create_node(NODE_TERMINAL, id.value));
...
{
    ParseNode *n = create_node(NODE_STATEMENT_TAIL, NULL);
    Token look = peek_ts(ts);
    if (look.type == TOKEN_COLON) {
        expect(ts, TOKEN_COLON);
        Token type_tok = expect(ts, TOKEN_KEYWORD);
        add_child(n, create_node(NODE_TERMINAL, type_tok.value));
...
{
    ParseNode *n = create_node(NODE_EXPRESSION, NULL);
...
ParseNode *parse_EXPRESSION_PRIME(TokenStream *ts)
{
    ...
    if (look.type == TOKEN_PLUS || look.type == TOKEN_MINUS || look.type == TOKEN_OPERATOR) {
        ParseNode *n = create_node(NODE_EXPRESSION_PRIME, NULL);
        Token op = get_from_ts(ts);
        add_child(n, create_node(NODE_TERMINAL, op.value));
        add_child(n, parse_TERMINAL(ts));
        add_child(n, parse_EXPRESSION_PRIME(ts));
        return n;
    }
...
{
    Token t = peek_ts(ts);
    ParseNode *n = create_node(NODE_TERMINAL, NULL);
